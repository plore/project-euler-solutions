triangle = """
75
95 64
17 47 82
18 35 87 10
20 04 82 47 65
19 01 23 75 03 34
88 02 77 73 07 63 67
99 65 04 28 06 16 70 92
41 41 26 56 83 40 80 70 33
41 48 72 33 47 32 37 16 94 29
53 71 44 65 25 43 91 52 97 51 14
70 11 33 28 77 73 17 78 39 68 17 57
91 71 52 38 17 14 91 43 58 50 27 29 48
63 66 04 68 89 53 67 30 73 16 69 87 40 31
04 62 98 27 23 09 70 98 73 93 38 53 60 04 23
"""

nums = [[int(s) for s in line.split(" ")] for line in triangle.strip().split("\n")]

# use dynamic programming

# initialization
# as on the edges there is no choice the optimum is the cumulative sum along the path
sums = [[0] * len(nums[i]) for i in range(len(nums))]
sums[0][0] = nums[0][0]
for i in range(1, len(nums)):
    sums[i][0] = nums[i][0] + sums[i - 1][0]
    sums[i][i] = nums[i][i] + sums[i - 1][i - 1]

# optimum in each "middle cell" is the optimum coming from either left or right
# above plus cell value
for y in range(2, len(nums)):
    for x in range(1, len(nums[y]) - 1):
        sums[y][x] = max(sums[y - 1][x], sums[y - 1][x - 1]) + nums[y][x]

print(max(sums[len(sums) - 1]))
